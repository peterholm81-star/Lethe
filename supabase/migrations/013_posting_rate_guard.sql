-- Migration: Upgrade insert_confession write guard
-- Adds a burst window on top of the existing 15-second gap check.
--
-- Policy now enforced (per auth.uid()):
--   • Gap limit:   minimum 15 seconds between any two posts
--   • Burst limit: maximum 3 posts within any rolling 5-minute window
--
-- Both limits only apply when v_user_id IS NOT NULL.
-- With anonymous auth active (ensureAnonSession), auth.uid() is always set,
-- so both limits are always enforced in production.
--
-- New exception prefix BURST_LIMIT is distinct from RATE_LIMIT so the
-- frontend can show the right message for each case.
...
BEGIN
  ...
  IF v_user_id IS NOT NULL THEN
    ...
    -- ── Gap check ─────────────────────────────────────────────────────────────
    -- Minimum 15 seconds between any two consecutive posts.
    ...
    -- ── Burst check ───────────────────────────────────────────────────────────
    -- Maximum 3 posts within any rolling 5-minute window.
    -- Uses the same confessions_user_rate_limit_idx index.
...
-- Re-affirm grants with fully-qualified signature to avoid ambiguity errors
-- when multiple overloaded versions of insert_confession exist in the database.
